<template>
    <v-app>
        <v-app-bar app color="primary" height="64px" dark>
            ...
            <v-spacer></v-spacer>
            <v-btn v-on:click="onClickUndo" v-show="isShowUndo" v-bind:disabled="!isEnableUndo" style="margin-left:8px;" text>
                <v-icon style="margin-right:5px;">fas fa-undo</v-icon>元に戻す
            </v-btn>
            <v-btn v-on:click="onClickRedo" v-show="isShowRedo" v-bind:disabled="!isEnableRedo" style="margin-left:8px;" text>
                <v-icon style="margin-right:5px;">fas fa-redo</v-icon>やり直す
            </v-btn>
            <v-btn v-on:click="onClickMemberChange" v-show="isShowMemberChange" style="margin-left:8px;" text>
                <v-icon style="margin-right:5px;">fas fa-exchange-alt</v-icon>選手交代
                <!-- <memberSvg></memberSvg> -->
            </v-btn>
            <v-btn v-on:click="onClickAnalyzeList" v-show="isShowAnalyzeList" v-bind:disabled="!isEnabledAnalyzeList" style="margin-left:8px;" text>
            ...
            <v-btn v-on:click="onClickAnalyzeMatch" v-show="isShowAnalyzeMatch" style="margin-left:8px;" v-bind:disabled="!isEnableAnalyzeMatch" text>
      ...
      <v-btn icon>
        ...
        <v-icon>fas fa-tools</v-icon>
            ...
            <v-btn v-bind:disabled="!isEnableSettings" v-on:click="onClickSettings" style="margin-left:8px;" text>
...
export default {
    ...
    methods: {
        ...
        routeScoreList() {
            ...
            this.isEnabledAnalyzeList = false;
            this.$router.replace({ path: "/scoreapp/scorelist" });
        },
        routeAnalyzeListPlayer(items) {
            ...
            this.$router.replace({ path: "/scoreapp/analyzelist" });
        },
        ...
        routeScoreInput(scoreId) {
            ...
            this.isShowAnalyzeMatch = false;
            this.isShowAnalyzeList = false;
            this.$router.replace({ path: "/scoreapp/scoreinput" });
        },
        routeScoreInputNew() {
            this.title = "スコア入力";
            this.target = "";
        ...
        routeSettings() {
            ...
            this.isShowAnalyzeMatch = false;
            this.isShowAnalyzeList = false;
            this.$router.replace({ path: "/scoreapp/settings" });
        },
        routeAnalyzeDetail(analyzePlayerData, isMatchFrom) {
            ...
            this.isShowAnalyzeMatch = false;
            this.isShowAnalyzeList = false;
            this.$router.replace({ path: "/scoreapp/analyzedetail" });
        },
        digestCallback(hex) {
            this.scoreId = hex;
            this.$router.replace({ path: "/scoreapp/scoreinput" });
